...
import { Mountain, LogOut, Plus, MapPin, CheckCircle, AlertTriangle, X, Timer } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
import { storage, type CheckIn, type EmergencyContact } from "@/lib/storage";
...
function StatusBadge({ status }: { status: CheckIn["status"] }) {
  ...
  return <span className={`px-2.5 py-0.5 rounded-full text-xs font-body font-semibold border ${cls}`}>{label}</span>;
}

function ActiveCheckIn({ checkIn, contacts, onSafe, onExtend, onCancel }: {
  checkIn: CheckIn;
  ...
  return (
    <motion.div
      ...
      className={`rounded-2xl border p-6 md:p-8 ${overdue ? "border-red-500/40 bg-red-500/5" : "border-accent/30 bg-accent/5"}`}
    >
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <p className="font-body text-xs uppercase tracking-[0.2em] text-muted-foreground mb-1">Active check-in</p>
          <h2 className="font-display text-2xl font-bold text-foreground">{checkIn.destination}</h2>
          <p className="font-body text-sm text-muted-foreground mt-1">
            {checkIn.activity_type} · Started {fmt(checkIn.started_at)}
          </p>
        </div>
        <button onClick={onCancel} className="text-muted-foreground hover:text-foreground transition-colors mt-1">
          <X className="h-5 w-5" />
        </button>
      </div>
        ...
        {overdue ? (
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="font-body text-sm text-muted-foreground">
              Your contacts would be alerted now once SMS is connected.
            </p>
          </div>
        ) : (
          <>
            <p className="font-body text-xs uppercase tracking-[0.2em] text-muted-foreground mb-3">Time remaining</p>
            ...
            <p className="font-body text-sm text-muted-foreground mt-3">
              Expected back by {fmt(checkIn.expected_return_at)}
            </p>
          </>
        )}
      </div>
      ...
      {contacts.length > 0 && (
        <div className="mb-6">
          <p className="font-body text-xs uppercase tracking-[0.2em] text-muted-foreground mb-3">Will alert</p>
      ...
      {checkIn.notes && (
        <p className="font-body text-sm text-muted-foreground mb-6 italic">"{checkIn.notes}"</p>
      )}

      <div className="flex flex-col sm:flex-row gap-3">
        <Button onClick={onSafe} className="flex-1 font-body font-bold text-base py-6" style={{ background: "var(--gradient-accent)" }}>
          <CheckCircle className="h-5 w-5 mr-2" />
          I'm Back Safe
        </Button>
        <Button onClick={onExtend} variant="outline" className="font-body font-semibold">
...
function NewCheckInForm({ onSave, onCancel }: {
  onSave: (data: Omit<CheckIn, "id" | "checked_in_at" | "status">) => void;
  onCancel: () => void;
}) {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      notes: notes.trim(),
      started_at: now,
      expected_return_at: new Date(Date.now() + hours * 3_600_000).toISOString(),
    });
  };
    ...
    >
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          <Input
            placeholder="Trail conditions, parking, anything useful"
            value={notes}
      ...
      <div className="mt-8">
        <Button
          onClick={handleSave}
          disabled={!canSave}
          className="w-full font-body font-bold py-5"
          ...
          <div key={t.id} className="flex items-center justify-between gap-4 rounded-xl border border-border bg-card px-5 py-4">
            <div className="min-w-0">
              ...
              <p className="font-body text-xs text-muted-foreground">
                {t.activity_type} · {fmt(t.started_at)}
              </p>
            </div>
...
export default function Dashboard() {
  const { user, isSetupComplete, signOut } = useAuth();
  ...
  useEffect(() => {
    ...
    if (!isSetupComplete) { navigate("/onboarding"); return; }
  }, [user, isSetupComplete, navigate]);

  const refresh = useCallback(() => {
    setContacts(storage.getContacts());
    const all = storage.getCheckIns();
    setActiveCheckIn(all.find((c) => c.status === "active") ?? null);
    setPastCheckIns(all.filter((c) => c.status !== "active"));
  }, []);

  useEffect(() => { refresh(); }, [refresh]);

  const handleNewCheckIn = (data: Omit<CheckIn, "id" | "checked_in_at" | "status">) => {
    storage.addCheckIn({ ...data, checked_in_at: null, status: "active" });
  ...
  const handleSafe = () => {
    ...
    storage.updateCheckIn(activeCheckIn.id, { status: "completed", checked_in_at: new Date().toISOString() });
    toast({ title: "Welcome back!", description: "Check-in complete. Stay safe out there." });
    refresh();
  };
  ...
  const handleExtend = () => {
    ...
    storage.updateCheckIn(activeCheckIn.id, { expected_return_at: newTime });
    toast({ title: "Extended 1 hour", description: `New expected return: ${fmt(newTime)}` });
    refresh();
  };

  const handleCancel = () => {
    if (!activeCheckIn) return;
    storage.updateCheckIn(activeCheckIn.id, { status: "cancelled" });
    ...
    <div className="min-h-screen bg-background">
      <nav className="flex items-center justify-between px-6 py-4 border-b border-border">
        ...
        <div className="flex items-center gap-4">
          <span className="font-body text-sm text-muted-foreground hidden sm:block">{user.name}</span>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10 space-y-8">
        <div className="flex items-center justify-between">
          ...
          {!activeCheckIn && !showForm && (
            <Button onClick={() => setShowForm(true)} className="font-body font-bold shrink-0" style={{ background: "var(--gradient-accent)" }}>
        ...
        {contacts.length === 0 && (
          <div className="flex items-start gap-3 rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-4">
            ...
            <div>
              <p className="font-body text-sm font-semibold text-foreground">No emergency contacts added</p>
              <p className="font-body text-xs text-muted-foreground mt-0.5">
                Add a contact so someone can be alerted if you don't check back in.
              </p>
            </div>
          </div>
        )}
        ...
        <AnimatePresence mode="wait">
          {activeCheckIn ? (
            ...
            <NewCheckInForm key="form" onSave={handleNewCheckIn} onCancel={() => setShowForm(false)} />
          ) : totalTrips === 0 ? (
            <motion.div
              key="empty"
              initial={{ opacity: 0 }}
            ...
            >
              ...
              <Button onClick={() => setShowForm(true)} className="font-body font-bold" style={{ background: "var(--gradient-accent)" }}>
                <Plus className="h-4 w-4 mr-2" />
                Start a check-in
              </Button>
            </motion.div>
